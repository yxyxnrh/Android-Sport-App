...
import com.WTK.ChangPao.commmon.utils.CountTimerUtil;
...
//        cmPasstime.setOnChronometerTickListener(chronometer -> cmPasstime.setText(formatseconds()));
    ...
    private void saveRecord() {
        ...
        ToastUtils.showShort("你的长跑数据会被安全保存在本地数据库");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            if (keyCode == KeyEvent.KEYCODE_BACK) { // 表示按返回键 时的操作
                //是否正在长跑记录数据
                if (ISSTARTUP) {
                    ToastUtils.showShort("退出请点击暂停，再结束长跑");
                    return true;

                }
                //是否有长跑记录
                if (null != record && null != record.getPathline() && !record.getPathline().isEmpty()) {
                    showTipDialog("你要强行退出吗?",
                            "强制退出会删除记录,想保留长跑数据,请点击结束",
                            new TipCallBack() {
                                @Override
                                public void confirm() {
                                    finish();
